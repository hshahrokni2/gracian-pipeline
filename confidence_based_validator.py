#!/usr/bin/env python3
"""
Confidence-Based Validator with Semantic Field Matching

This validator solves the schema mismatch problem by:
1. Using SemanticFieldMatcher to find fields regardless of path
2. Applying confidence-based comparison (fuzzy matching for names, tolerance for numbers)
3. Working with any schema structure (agent-grouped, Pydantic, semantic)
"""
...
def normalize_value(val: Any) -> Any:
    ...
    # Handle lists (compare lengths and normalized elements)
    if isinstance(val, list):
        return [normalize_value(v) for v in val]

    # Handle dicts (normalize nested structure)
...
def compare_values(extracted: Any, ground_truth: Any, tolerance: float = 0.05) -> Tuple[bool, str]:
    """
    Compare extracted value with ground truth value using confidence-based matching.
    ...
    # String comparison (case-insensitive, fuzzy matching for Swedish names)
    if isinstance(norm_ground_truth, str):
        if isinstance(norm_extracted, str):
            ...
            # Fuzzy match (allows for minor OCR/extraction errors in Swedish names)
...
def main():
    """Run confidence-based validation with semantic field matching."""

    # Paths
    pdf_path = "SRS/brf_198532.pdf"
    gt_path = "ground_truth/brf_198532_comprehensive_ground_truth.json"  # Use semantic ground truth
    ...
    print("=" * 80)
    print("CONFIDENCE-BASED VALIDATION WITH SEMANTIC FIELD MATCHING")
    print("=" * 80)
    ...
    print("\n🔍 Initializing semantic field matcher...")
    matcher = SemanticFieldMatcher()
    print(f"   ✓ Loaded {len(matcher.field_synonyms)} canonical field mappings")

    # Validation with semantic matching
    print("\n📊 Running validation with semantic field matching...\n")

    total_fields = 0
    matched_fields = 0
    field_results = []

    # Iterate through ground truth fields and find them in extraction using semantic matcher
    def validate_nested_dict(gt_dict: Dict, prefix: str = ""):
        ...
        for key, gt_value in gt_dict.items():
            ...
            # Use semantic matcher to find field in extraction
            ...
            if ext_value is not None:
                ...
                print(f"{status:20} | {field_path:40} | confidence={confidence:.2f}")
            else:
                field_results.append({
                    "field": field_path,
                    "status": "✗ NOT FOUND",
                ...
                print(f"{'✗ NOT FOUND':20} | {field_path:40} | confidence=0.00")

            # Recurse for nested dicts
            if isinstance(gt_value, dict):
                validate_nested_dict(gt_value, field_path)

    validate_nested_dict(ground_truth)
    ...
    # Save results
    output_path = "week3_day5_confidence_validation_results.json"
